...
// 変更: V6.3.0 fix: 印刷範囲をB〜I列（罫線の内側）に限定。
//                    従来は印刷範囲未設定でJ〜N列の作業列まで印刷対象になり、
//                    その分だけ横幅に合わせる縮小がかかって表が小さくなっていた。
// ============================================================
...
export async function POST(req: NextRequest) {
  ...
  // 印刷範囲は罫線の内側（B〜I列）だけにする。
  // J〜N列は労務比率・搬入除外計などの作業列なので印刷しない。
  // これで用紙の横幅に合わせたときに罫線が縁まで広がる。
  // ExcelJSはprintAreaの各辺の先頭に'$'を足すだけなので、行にも'$'を付けて渡す。
  // （'B1:I58'と渡すと'$B1:$I58'になり、行が絶対参照にならず印刷範囲が効かない）
  ws.pageSetup.printArea = `B$1:I$${totalRows}`
