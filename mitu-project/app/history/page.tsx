...
type EstimateItem = {
  ...
  exclude_hakobi: boolean|null; night_work: boolean|null
  labor_rate: number|null; night_deep_rate: number|null
}
type MasterItem = {
...
export default function HistoryPage() {
  ...
  const deleteArchivedVersions = async (baseId: number) => {
    const group = estimates.filter(e => (e.base_id || e.id) === baseId)
    const newestId = Math.max(...group.map(e => e.id))
    ...
    // 明細を先に消す（見積だけ消すと明細が孤児として残るため）
    const { error: itemsError } = await supabase.from('estimate_items').delete().in('estimate_id', ids)
    if (itemsError) { alert('明細の削除に失敗しました: ' + itemsError.message); setDeletingArchived(false); return }
    const { error: estError } = await supabase.from('estimates').delete().in('id', ids)
    if (estError) { alert('見積の削除に失敗しました: ' + estError.message); setDeletingArchived(false); return }
    setDeletingArchived(false)
    setShowArchived(false)
    alert(`${targets.length}件を削除しました。`)
    await loadEstimates()
  }
  ...
  const insertRowBefore = (sectionId: string, rowId: string) => {
    const row = newRow()
    setSections(prev => prev.map(s => {
      if (s.id !== sectionId) return s
      const idx = s.rows.findIndex(r => r.id === rowId)
    ...
    setSections(prev => prev.map(s => s.id !== sectionId ? s : {
      ...s, rows: s.rows.map(r => {
        ...
        if (field === 'nightWork') {
          ...
          return { ...r, [field]: newVal, note1, note2, note3, laborRate }
        }
  ...
  const getKeihiCost = (section: Section) => {
    if (section.name === '特殊仮設工事') return 0
                      ...
                      <tr key={row.id} className="border-t align-top">
                        ...
                        <td className="p-1 align-top">
                          <div className="flex flex-col gap-0.5 items-center pt-1">
                            {rowIdx === 0 && (
                              <button onClick={() => insertRowBefore(section.id, row.id)}
                            ...
                            <button onClick={() => deleteRow(section.id, row.id)}
                              ...
                              title="この行を削除">➖</button>
                            <button onClick={() => insertRowAfter(section.id, row.id)}
                              className="w-7 h-6 bg-green-100 hover:bg-green-200 text-green-700 rounded text-xs font-bold"
                              title="下に行挿入">＋</button>
                          </div>
                        </td>
